...
if ($env:BuildSemanticVersion -eq $null) {
    ...
    $env:BuildSemanticVersion = "0.0.0-dev-" + $autoVersion
...
Get-ChildItem */*/project.json | %{ echo $_.FullName } | %{
    ...
    $content = $content.Replace("0.0.0-dev", "$env:BuildSemanticVersion")
...
Get-ChildItem */*/project.json | %{ echo $_.FullName } | %{
    $content = get-content "$_"
    $content = $content.Replace("$env:BuildSemanticVersion", "0.0.0-dev")
    set-content "$_" $content -encoding UTF8
}
